...
function db(): PDO
{
    ...
    $tableExists = (int) $pdo->query("SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' AND name = 'transactions'")->fetchColumn();
    if ($tableExists > 0) {
        $columns = $pdo->query("PRAGMA table_info(transactions)")->fetchAll();
